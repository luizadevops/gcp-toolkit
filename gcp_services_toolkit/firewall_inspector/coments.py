...
def check_ports_match(rule_ports: Sequence[str], config_ports: List[str]) -> bool:
    ...
    try:
        ...
        logging.warning(f"{_tool_name_for_logging()}: Invalid port format. rule_ports={rule_ports}, config_ports={config_ports}. No match.")
        return False
...
def list_firewall_rules(project_id: str) -> List[Firewall]:
    ...
    try:
        ...
        logging.info(f"{_tool_name_for_logging()}: Listed {len(firewalls_list)} firewall rules for project '{project_id}'.")
    except google_exceptions.Forbidden:
        logging.error(f"{_tool_name_for_logging()}: Permission denied to list firewall rules in '{project_id}'. Requires 'compute.firewalls.list'.")
    except google_exceptions.NotFound:
        logging.error(f"{_tool_name_for_logging()}: Project '{project_id}' not found or Compute Engine API not enabled.")
    except Exception as e:
        logging.error(f"{_tool_name_for_logging()}: Failed to list firewall rules for project '{project_id}': {e}", exc_info=True)
...
def is_rule_overly_permissive(rule: Firewall, fw_config_params: Dict) -> Tuple[bool, str]:
    ...
    if target_tags_to_ignore and set(rule.target_tags).intersection(target_tags_to_ignore):
        logging.debug(f"{_tool_name_for_logging()}: Rule '{rule.name}' (source: '{source_ip_alert}') skipped: matches ignored target_tags: {list(rule.target_tags)}")
        return False, ""
    
    if target_sas_to_ignore and set(rule.target_service_accounts).intersection(target_sas_to_ignore):
        logging.debug(f"{_tool_name_for_logging()}: Rule '{rule.name}' (source: '{source_ip_alert}') skipped: matches ignored target_service_accounts: {list(rule.target_service_accounts)}")
        return False, ""

    if not rule.allowed:
        logging.debug(f"{_tool_name_for_logging()}: Rule '{rule.name}' has source '{source_ip_alert}' but no 'allowed' protocols/ports.")
        return False, ""
    ...
    for allowed_item in rule.allowed:
        ...
        for criterion_dict, config_ports_for_criterion in matching_criteria_from_config:
            if not config_ports_for_criterion:
                return True, f"Allows {protocol.upper()} on ALL ports (config: protocol '{criterion_dict.get('protocol')}') from '{source_ip_alert}'."
            if check_ports_match(rule_ports, config_ports_for_criterion):
                return True, (f"Allows {protocol.upper()} on ports ({rule_ports if rule_ports else 'ALL'}) "
                              f"from '{source_ip_alert}' matching config ports "
                              f"'{config_ports_for_criterion}' for protocol '{criterion_dict.get('protocol')}'.")
    return False, ""
...
def delete_firewall_rule(project_id: str, rule_name: str, dry_run: bool = True) -> bool:
    ...
    if dry_run:
        logging.info(f"{_tool_name_for_logging()} - [DRY-RUN]: Would delete firewall rule '{rule_name}' in project '{project_id}'.")
        return True

    logging.info(f"{_tool_name_for_logging()}: Attempting to delete rule '{rule_name}' in project '{project_id}'.")
    try:
        operation = client.delete(project=project_id, firewall=rule_name)
        logging.info(f"{_tool_name_for_logging()}: Delete operation for rule '{rule_name}' in project '{project_id}' initiated. Operation ID: {operation.name}")
        return True
    except google_exceptions.NotFound:
        logging.warning(f"{_tool_name_for_logging()}: Rule '{rule_name}' not found in project '{project_id}'. Might be already deleted.")
        return False
    except google_exceptions.Forbidden:
        logging.error(f"{_tool_name_for_logging()}: Permission denied to delete rule '{rule_name}' in project '{project_id}'. Requires 'compute.firewalls.delete'.")
        return False
    except Exception as e:
        logging.error(f"{_tool_name_for_logging()}: Unexpected error deleting rule '{rule_name}' in project '{project_id}': {e}", exc_info=True)
        return False

def run_firewall_inspector(project_id: str, fw_config_params: Dict,
                           attempt_deletion: bool, is_global_dry_run: bool):
    tool_name = _tool_name_for_logging() # Use helper for consistent naming
    ...
    if attempt_deletion and not is_global_dry_run:
        logging.warning(f"{tool_name}: --delete active & global --dry-run OFF. REAL DELETIONS WILL BE ATTEMPTED IF CONFIRMED.")
        try:
            confirm = input(f"{tool_name}: CONFIRM REAL DELETIONS for flagged firewall rules? (yes/no): ")
            if confirm.lower() == 'yes':
                logging.info(f"{tool_name}: User confirmed real deletions.")
                effective_dry_run_for_delete_action = False
            else:
                logging.info(f"{tool_name}: User CANCELLED real deletions. No rules will be deleted.")
                proceed_with_delete_actions = False
        except EOFError:
            logging.error(f"{tool_name}: --delete used in non-interactive environment without global --dry-run. REAL DELETIONS ABORTED.")
            proceed_with_delete_actions = False
    elif attempt_deletion and is_global_dry_run:
        logging.info(f"{tool_name}: --delete active, but global --dry-run is also active. Deletions will be SIMULATED.")
    ...
    if all_firewalls is not None:
        ...
        if proceed_with_delete_actions: # Only report on delete actions if they were intended
            action_verb = "simulated" if effective_dry_run_for_delete_action else "initiated"
            if actions_taken_on_rules > 0 or flagged_rules_count > 0 : # Avoid logging if no rules were even flagged for action
                logging.info(f"{tool_name}: {actions_taken_on_rules} flagged rule(s) had deletion {action_verb}.")
            elif flagged_rules_count == 0 :
